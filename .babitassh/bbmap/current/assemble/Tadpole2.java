...
//						outstream.println(new String(bb.array, bb.length()-31, 31));
	...
	public boolean isJunk(Read r){
		boolean junk=isJunk(r, localRightCounts.get(), localKmer.get());
		return junk;
	}
	...
	@Override
	public boolean hasLowCountKmers(Read r, int minDepth){
		return hasLowCountKmers(r, minDepth, localKmer.get());
	}
	
	public boolean hasLowCountKmers(Read r, int minDepth, Kmer kmer){
		final int blen=r.length();
		if(blen<kbig){return true;}
		final byte[] bases=r.bases;
