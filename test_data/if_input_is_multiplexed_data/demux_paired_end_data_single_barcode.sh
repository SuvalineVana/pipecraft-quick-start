...
tdiffs=$"6"
processors=$"1"
tag_attached_to=$"fwd" #options = rev, fwd
###############################
...
###### SINGLE-END BARCODES DEMUX; PAIRED-END DATA (R1 and R2)
...
if [[ $tag_attached_to == "rev" ]]; then #If tag with rev primer, then reverse complement R1R2.fasta
	vsearch --fastx_revcomp R1R2.fasta --fastaout R1R2.rc.fasta
	#Rename fasta file
	mv R1R2.rc.fasta R1R2.fasta
fi
#Note: if tag with fwd primer, then do nothing here.

###Demultiplex fasta file based on barcodes file
mothur "#trim.seqs(fasta=R1R2.fasta, oligos=$barcodes_file, bdiffs=$bdiffs, pdiffs=$pdiffs, tdiffs=$tdiffs, processors=$processors)"
...
#demux R1
echo "python demux_FASTX_based_on_groups.py $inputR1.$format $format"
...
cd ..
#demux R2
echo "python demux_FASTX_based_on_groups.py $inputR2.$format $format"
